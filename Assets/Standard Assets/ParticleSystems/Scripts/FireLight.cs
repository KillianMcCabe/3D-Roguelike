...
namespace UnityStandardAssets.Effects
{
    public class FireLight : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (m_Burning)
            {
                m_Light.intensity = startIntensity + 2 * Mathf.PerlinNoise(m_Rnd + Time.time, m_Rnd + 1 + Time.time*1);
                ...
                //transform.localPosition = Vector3.up * .125f + new Vector3(x, y, z) * .125f;
                transform.localPosition = new Vector3(x, y, z) * .125f;
            }
        }
